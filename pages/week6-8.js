// pages/week6-8.js — Protected: must be logged in AND paid
...
function CourseContent() {
  ...
  // ════════════════════════════════════════════════════
  // PASTE YOUR ORIGINAL week6-8.js CONTENT BELOW HERE
  // ════════════════════════════════════════════════════
  ...
  return (
    <div style={{ fontFamily: "'Jost', sans-serif", background: '#FAF6F0', minHeight: '100vh' }}>
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '4rem 2rem', textAlign: 'center' }}>
        <div style={{ fontSize: '3rem', marginBottom: '1rem' }}>🌿</div>
        <h1 style={{ fontFamily: "'Cormorant Garamond', serif", color: '#3D5A3E', fontWeight: 300, fontSize: '2.5rem' }}>Weeks 6–8: Integration & Embodiment</h1>
        ...
        <p style={{ color: '#7A6A5A', lineHeight: 1.7, marginTop: '1rem', fontSize: '0.9rem', background: 'white', border: '1px solid #E0D5C5', borderRadius: 8, padding: '1rem' }}>
          ⚠️ To complete setup: open your current <strong>week6-8.js</strong> file, copy the full content of your component's return statement, and paste it into the <strong>CourseContent</strong> component above this message. Then delete this placeholder paragraph.
        </p>
      </div>
    </div>
  )
...
export default function Week6_8() {
  const { user, loading, isPaid, isAdmin } = useAuth()
  const router = useRouter()

  useEffect(() => {
    if (!loading && !user) {
      router.replace('/login?redirect=/week6-8')
    }
  }, [loading, user, router])
